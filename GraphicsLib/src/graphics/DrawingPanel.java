...
class DrawingPanel extends FileFilter
    ...
    // FT: method to check if the frame is closed
    public boolean isClosed() {
    	return instances == 0;
    }
    
    // FT: method to close the frame & the window
    public void close() {
    	if (instances > 0) {
    ...
    /**
     * Probe the pixel color at the given coordinates. Return BLACK in all error conditions
     * @param x - x pixel coordinate
     * @param y - y pixel coordinate
     * @return Color of the pixel at the (x, y) coordinate, or BLACK in case of error.
     */
    public Color getColor(int x, int y) {
        Color c = Color.BLACK;
        if (!isClosed()) {
            BufferedImage image2 = new BufferedImage(1, 1, image.getType());
            Graphics g = image2.getGraphics();
            g.setColor(backgroundColor);
            g.fillRect(0, 0, 1, 1);
            g.drawImage(image, 0, 0, 1, 1, x, y, x+1, y+1, panel);
            int rgb = image2.getRGB(0, 0);
            c = new Color(rgb);
        }
        return c;
    }
